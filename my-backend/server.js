...
// MongoDB Connection
const mongoURI =
  "mongodb+srv://doopgk:EOH0DY8xwJ2Uy4AG@capangular.xjtne.mongodb.net/?retryWrites=true&w=majority&appName=capangular";
mongoose
...
app.post("/api/verify-otp", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .send({ error: "Error during OTP verification", details: error.message });
  }
});
...
app.post("/api/products", upload.single("image"), async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send({ error: "Error adding product", details: error.message });
  }
});
...
app.put("/api/products/:id", async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send({ error: "Error updating product", details: error.message });
  }
});
  ...
  new mongoose.Schema(
    {
      ...
      cartProducts: [
        {
          // Assuming this structure for cart products
          productId: {
            type: mongoose.Schema.Types.ObjectId,
            ref: "Product",
            required: false,
...
app.post("/api/service-booking", async (req, res) => {
  try {
    const { fullName, email, state, city, serviceType } = req.body;

    // Create a new service booking document
    ...
    const imagePath = path.join(__dirname, '/src/assets/images/logof.png'); 
    ...
    attachments: [
        {
          filename: 'service.png', // The name of the file as it will appear to the recipient
          path: imagePath // The path to the file on your server
        }
      ]
    };
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      ...
      res
        .status(201)
        .send({
          message: "Service booking successful! Confirmation email sent.",
        });
    });
  } catch (error) {
    ...
    res
      .status(500)
      .send({
        error: "Error processing service booking",
